    ...
    for (const option of cmd.options) {
      ...
      if (option.choices) {
        for (const choice of option.choices) {
          if (choice.name && choice.name.length > 110) {
            validationErrors.push(
              `Command ${cmd.name} option ${option.name} choice name too long`
            );
          }
          if (choice.value && String(choice.value).length > 100) {
            validationErrors.push(
              `Command ${cmd.name} option ${option.name} choice value too long`
            );
          }
        }
      }
      ...
      for (const subOption of option.options) {
        if (subOption.name && subOption.name.length > 32) {
          validationErrors.push(
            `Command ${cmd.name} subcommand option name too long`
          );
        }
        if (subOption.description && subOption.description.length > 110) {
          validationErrors.push(
            `Command ${cmd.name} subcommand option description too long`
          );
        }
      }
    }
...
export async function registerCommands(client, options = {}) {
  ...
  try {
    ...
    const guildId = process.env.GUILD_ID || process.env.TEST_GUILD_ID || null;

    validateCommands(commands);
    const commandsToRegister = prepareCommandsForRegistration(commands);

    // Registro INSTANTÁNEO en tu servidor si hay GUILD_ID
    if (guildId && clientId) {
      logger.info(
        `Registering ${commandsToRegister.length} commands to guild ${guildId} (instant)...`
      );
      ...
      logger.info(
        `Successfully registered ${commandsToRegister.length} guild commands`
      );
      return;
    }

    // Si no hay GUILD_ID, registra global (puede tardar)
